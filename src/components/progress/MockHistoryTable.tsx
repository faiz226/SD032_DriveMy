...
export function MockHistoryTable({ data }: MockHistoryTableProps) {
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-muted text-muted-foreground">
            <tr>
              ...
              <th scope="col" className="px-4 py-3">{t("progress.table.set")}</th>
              ...
              <th scope="col" className="px-4 py-3">{t("progress.table.status")}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {paginatedData.map((row) => (
              <tr key={row.id} className="bg-card hover:bg-muted/50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${row.passed ? 'bg-success/10 text-success' : 'bg-destructive/10 text-destructive'}`}>
                    {row.passed ? t("progress.pass") : t("progress.fail")}
                  </span>
                </td>
              </tr>
            ))}

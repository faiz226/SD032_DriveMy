...
export class ThreePointTurnScene extends Phaser.Scene {
  ...
  constructor() { super({ key: "three-point-turn" }); }
  ...
  create(): void {
    ...
    const cg = this.add.rectangle(width / 4, height / 2, 40, 24, 0x3b82f6);
    ...
    this.obstacles.add(this.add.rectangle(width / 2, height / 2 - 60, width, 8, 0x64748b));
    this.obstacles.add(this.add.rectangle(width / 2, height / 2 + 60, width, 8, 0x64748b));
    this.finishZone = this.add.zone(width / 4, height / 2, 60, 80);
    ...
    this.physics.add.overlap(this.car, this.finishZone, () => {
      const n = ((this.carAngle % 360) + 360) % 360;
      if (this.speed < 2 && n > 150 && n < 210) this.handleFinish();
    });

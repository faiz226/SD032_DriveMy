...
/**
 * ParallelParkingScene — Top-down view.
 * Car must parallel park between two parked vehicles.
 *
 * Key: "parallel-parking"
 */
export class ParallelParkingScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // Car
    const carGraphic = this.add.rectangle(width / 2, height - 80, 40, 24, 0x3b82f6);
    ...
    // Parked cars (obstacles)
    this.obstacles = this.physics.add.staticGroup();
    const frontCar = this.add.rectangle(width / 2 - 80, height / 2, 40, 24, 0xef4444);
    const rearCar = this.add.rectangle(width / 2 + 80, height / 2, 40, 24, 0xef4444);
    const curb = this.add.rectangle(width / 2, height / 2 - 40, 200, 8, 0x64748b);
    this.obstacles.add(frontCar);
    this.obstacles.add(rearCar);
    this.obstacles.add(curb);

    // Finish zone (between the two parked cars)
    this.finishZone = this.add.zone(width / 2, height / 2, 100, 40);
  ...
  private handleFinish(): void {
    ...
    this.hudCallbacks?.onFinish({
      ...
      errors: this.errors, stallCount: this.stallCount,
      rollbackCm: this.rollbackCm, passed: this.errors <= 3,
    });
    this.scene.pause();
  }
}
